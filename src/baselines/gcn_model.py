...
def train_gcn_model(
    ...
    print("=" * 70)
    print("Step 5: Training GCN model...")
...
def evaluate_gcn_model(model, X_test, y_test, test_adj, device):
    print("=" * 70)
    print("Step 6: Evaluating GCN model...")
...
def plot_loss_curve(loss_history, figure_dir):
    ...
    figure_path = figure_dir / "gcn_loss_curve.png"
...
def plot_transaction_subgraph(edge_index, figure_dir, max_nodes=80, max_edges=200):
    ...
    plt.title("Sampled Transaction Subgraph for GCN")
    plt.axis("off")

    figure_path = figure_dir / "gcn_transaction_subgraph.png"
    plt.savefig(figure_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def save_results(output_dir, metrics_df, test_df, y_pred, y_prob):
    ...
    metrics_path = output_dir / "gcn_metrics.csv"
    pred_path = output_dir / "gcn_predictions_sample.csv"
...
# 整体运行流程
# 顺序为：读取数据 -> 节点特征准备 -> 图构建 -> GCN训练 -> 评估 -> 画图 -> 保存结果
def run_gcn_baseline(
    ...
    train_adj = build_normalized_adj(
        edge_index=train_edge_index,
        num_nodes=X_train.shape[0],
        device=device
    )

    test_adj = build_normalized_adj(
        edge_index=test_edge_index,
        num_nodes=X_test.shape[0],
        device=device
    )

    model, loss_history = train_gcn_model(
        X_train=X_train,
        y_train=y_train,
        train_adj=train_adj,
    ...
    metrics_df, y_pred, y_prob = evaluate_gcn_model(
        model=model,
        X_test=X_test,
        y_test=y_test,
        test_adj=test_adj,
    ...
    print("=" * 70)
    print("GCN baseline completed successfully.")
...
if __name__ == "__main__":
    ...
    model, metrics_df = run_gcn_baseline(
        processed_dir=processed_dir,
        output_dir=output_dir,
        train_file="train_split.csv",
        test_file="test_split.csv",
