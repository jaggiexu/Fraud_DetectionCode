...
# 训练 Random Forest 模型
# 关键参数说明：
# 1. n_estimators 控制树的数量
# 2. max_depth 限制树深，避免训练过慢
# 3. class_weight='balanced' 缓解类别不平衡
...
def save_results(output_dir, metrics_df, test_df, y_pred, y_prob):
    ...
    metrics_path = output_dir / "rf_metrics.csv"
    pred_path = output_dir / "rf_predictions_sample.csv"
...
if __name__ == "__main__":
    ...
    model, metrics_df = run_random_forest_baseline(
        processed_dir=processed_dir,
        output_dir=output_dir,
        train_file="train_split.csv",
        test_file="test_split.csv",
